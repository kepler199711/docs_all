主播需要先调用 [getGameList](@getGameList) 接口，传入 pageIndex（分页查询的标识位，第一次填 0 ）、pageSize（获取游戏数量）和 gameMode（游戏模式），获取“主播带玩模式”的游戏列表，从返回结果（包括游戏 ID、游戏名称、游戏描述和缩略图）中选择游戏（游戏 ID）。
