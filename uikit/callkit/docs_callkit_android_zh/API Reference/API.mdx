...
用于在房间内发送自定义命令的接口。最大长度为1024字节。
房间内的其他用户可以通过调用CallEvents.addInRoomCommandListener(ZegoInRoomCommandListener)来监听这个命令。当toUserList为[null]时，SDK会将自定义命令发送给房间内的所有用户。
