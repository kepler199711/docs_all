...
### 5 The invitation notification message-box did not pop up in background mode(only Android)

- Check if `SystemChannels.lifecycle.setMessageHandler` is used
  
  Due to the fact that `SystemChannels.lifecycle.setMessageHandler` can only be listened to once in the entire app.
  
  if you also want to listen to this event, please use `ZegoUIKit().adapterService().registerMessageHandler(youHandler)` to handle, Of course, remember that when the page is disposed, you need to `ZegoUIKit().adapterService().unregisterMessageHandler(youHandler)`.
