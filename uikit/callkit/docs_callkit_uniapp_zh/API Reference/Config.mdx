...
| 属性           | 类型    | 描述                                                   |
| -------------- | ------- | ------------------------------------------------------ |
| enableHWPush   | Boolean | 是否开启华为推送布尔值。                               |
| enableMiPush   | Boolean | 是否开启小米推送的布尔值。                             |
| enableVivoPush | Boolean | 是否开启 vivo 推送的布尔值。                           |
| enableOppoPush | Boolean | 是否开启 oppo 推送的布尔值。                           |
| miAppID        | Sting   | 小米推送所需 AppID。                                   |
| miAppKey       | Sting   | 是否开启小米推送的布尔值。                             |
| oppoAppID      | Sting   | oppo 推送所需 AppID。                                  |
| oppoAppSecret  | Sting   | oppo 推送所需的AppSecret。                             |
| oppoAppKey     | Sting   | oppo 推送所需的 AppKey。                               |
| vivoAppID      | Sting   | vivo 推送所需的 AppID。                                |
| vivoAppKey     | Sting   | vivo 推送所需的 AppKey。                               |
| hwAppID        | Sting   | 华为推送所需的 AppID。                                 |
| appType        | Number  | 用于指定当前项目对应 ZEGO 控制台的第几套离线推送配置。 |
...
| 属性    | 类型   | 描述                             |
| ------- | ------ | -------------------------------- |
| title   | String | 用于设置推送标题。非必填         |
| message | String | 用于设置离线推送的内容。非必填 |
| resourcesID | String | 用于映射多个厂商进阶配置的离线推送字段，将多个厂商的推送配置抽象为一种您自定的推送策略。 |
