...
- `onOutgoingCallRejectedCauseBusy?: (callID: string, callee: ZegoUser) => void`：当被叫用户拒绝呼叫邀请时（被呼叫者忙碌），主叫用户将通过此回调接收到通知。
...
**音视频通话 UIKit** 提供了一组默认的通话页面行为和样式，如果您需要自定义通话界面，您可以通过配置 [ZegoUIKitPrebuiltCallConfig](../API%20Reference/Config.mdx) 属性进行自定义。
