...
interface to send custom command in room.Max Length is 1024 bytes
Other users in the room can listen to this by CallEvents.addInRoomCommandListener(ZegoInRoomCommandListener).When 'toUserList' is [null], the SDK will send custom command to all users in the room
```java title="function prototype"
