...
- func `requireRoomForegroundView`() -> UIView?: Add a view on the audio and video call interface. The size of the view is determined by the view.frame passed by the customer. Add a view on the top layer of the audio and video call interface
...
extension CallInvitationVC: ZegoUIKitPrebuiltCallInvitationServiceDelegate {
 ...
 func requireConfig(_ data: ZegoCallInvitationData) -> ZegoUIKitPrebuiltCallConfig {
  ...
  // Add a view to the upper layer of the audio and video call interface. The size of the view is determined by the view.frame passed in by the client
