...
- func `requireRoomForegroundView`() -> UIView?: Add a view on the top layer of the audio and video call interface. The size of the view is determined by the `view.frame` passed by you. As shown in the following image, the foreground view can set to red and covering the entire interface.
