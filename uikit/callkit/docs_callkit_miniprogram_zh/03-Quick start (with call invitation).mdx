    ...
    <Step title="调用 UIKit 组件。">
        <Tabs>
            ...
            <Steps>
                ...
                <Warning title="Warning">设置房间配置的 <code>mode</code> 参数为 <code>ZegoCallScenario.CALL_INVITATION</code> 和 <code>globalPagePath</code> 参数为 UIKit 组件的存储路径后才能发起呼叫和接受呼叫。</Warning>
