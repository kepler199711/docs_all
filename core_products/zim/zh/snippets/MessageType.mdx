...
- 组合消息：通常用于发送图文组合的消息。限制说明：总的 item 数量不超过 20 个；图片类型的数量不超过 10 个；文件、语音、视频、自定义消息的数量各最多 1 个；其中图片、语音、文件、视频的文件大小和格式限制分别与对应的富媒体文件类型消息保持一致。
...
<td>组合消息。仅支持包含：文本、自定义消息、图片、文件、语音、视频的组合。</td>
</tr>
<tr data-row-level="6">
