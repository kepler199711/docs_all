...
客户端需要使用各自的用户信息进行登录。调用 [login](@login__2) 接口，传入用户信息 [ZIMUserInfo](@-ZIMUserInfo) 对象，进行登录。
...
相关接口：[sendMediaMessage](@sendMediaMessage)
</td>
</tr>
<tr data-row-level="6">
...
客户端 B 登录 ZIM 后，将会收到在 [setEventHandler](@setEventHandler) 回调中设置的 [onReceivePeerMessage](@onReceivePeerMessage) 监听接口，收到客户端 A 发送过来的消息。
