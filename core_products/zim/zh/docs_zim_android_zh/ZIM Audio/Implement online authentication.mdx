...
开发者服务端通过公共网关接口，构造消息体，直接发送请求到 ZEGO 服务端，申请鉴权文件。消息体结构请参考服务端 API - 获取鉴权信息文档的 [接口原型](/zh/zim-server/zim-audio/obtain-a-license#接口原型)、[请求参数](/zh/zim-server/zim-audio/obtain-a-license#请求参数) 和 [请求示例](/zh/zim-server/zim-audio/obtain-a-license#请求示例)。

### 2 ZEGO 服务端返回鉴权文件 License

ZEGO 服务端返回的消息请参考服务端 API - 获取鉴权信息文档的 [响应参数](/zh/zim-server/zim-audio/obtain-a-license#响应参数) 和 [响应示例](/zh/zim-server/zim-audio/obtain-a-license#响应参数)。
