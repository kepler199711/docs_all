...
protected void onNotificationClicked(Context context, ZPNsMessage message) {
    ...
    // 获取透传消息（即跳转目标页面的路径）
    // 与发送消息时传入的 ZIMPushConfig > payload 字段内容一致,请参考实现离线推送,使用 ZIM SDK 实现离线推送功能章节。
...
:::
<Step title="配置透传字段">
消息发送者在发送消息时，需要通过 `ZIMPushConfig > payload` 参数设置透传字段（例如需要跳转到的目标页面路径）。相关代码请参考 [实现离线推送 - 使用 ZIM SDK 实现离线推送功能 - 场景 1：发送单聊消息时使用离线推送功能](./Implement%20offline%20push%20notification.mdx#场景-1发送单聊消息时使用离线推送功能) 章节。
</Step>
<Step title="触发回调">
:::if{props.platform=undefined}
