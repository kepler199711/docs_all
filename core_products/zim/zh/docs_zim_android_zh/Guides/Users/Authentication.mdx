...
在 Token 过期前 30 秒，SDK 会通过 [tokenWillExpire\|_blank](@tokenWillExpire) 回调发出通知。若登录成功后 Token 有效期不足 30 秒，则会立即回调。

收到该回调后，开发者需要从自己的服务端获取新的有效 Token，并调用 SDK 提供的 [renewToken\|_blank](@renewToken) 接口更新 Token。

<Note title="Note">

当 Token 过期且未得到更新时，用户会掉线，收到 [connectionStateChanged\|_blank](@connectionStateChanged) 回调，其中 `event` 为 `TokenExpired`，`state` 为 `Disconnected`。
</Note>
...
| 方法 | 描述 |
|-------|--------|
| [login\|_blank](@login__2) | 登录 ZIM 服务。  |
| [renewToken\|_blank](@renewToken) | 更新鉴权 Token。 |
...
std::string token = 'xxxxxxxxxx'; // 请求开发者服务端获取
...
:::if{props.platform="undefined|windows"}
...
| [onTokenWillExpire](@onTokenWillExpire) | Token 过期回调。 |
