...
| :-- | :-- | 
| 双平台登录 | 支持用户在 Windows、Mac、Linux、Android、iOS 和 iPad 的其中 1 个平台上登录帐号，并同时保持帐号在 Web 平台在线。 |
| 多平台登录 | 是指支持用户同时在 Windows、Mac、Linux、Android、iOS、iPad、Web、小程序多平台登录帐号，支持以下配置：<br/><ul><li>同一平台同时多设备、实例登录。</li><li>配置互踢逻辑，当前仅支持 android、iOS 设备互踢，Windows、Mac 设备互踢。</li></ul> |


## 实现流程

联系 ZEGO 技术支持，开通多端登录服务，配置多端登录之后，即可在多端调用 [login\|_blank](@login__2) 登录 ZIM SDK，实现流程与单端登录无异。如何登录，详情请参考 [实现基本消息收发\|_blank](!IM-Quick_Starts/Implementation_of_sending_and_receiving#login) 的“登录 ZIM“。 

当登录设备已达到策略规定的上限时，如果在新设备登录帐号，原设备中最早登录的帐号会被踢下线，并通过 [onConnectionStateChanged\|_blank](@onConnectionStateChanged) 回调得知 event 为 `KickedOut` 。例如：登录策略规定最多只能在 3 台设备上登录，已在设备 A、B、C（按登录时间排序）登录，如果在设备 D 上登录帐号，则设备 A 上的帐号会被踢下线。

当帐号被踢下线后，建议调用 [logout\|_blank](@logout) 接口登出 ZIM 服务，用户界面切换为登录界面。
...
ZIM SDK 为多端登录用户提供了用户信息同步的功能。当用户在一端通过 [updateUserName\|_blank](@updateUserName)、[updateUserAvatarUrl\|_blank](@updateUserAvatarUrl)、[updateUserExtendedData\|_blank](@updateUserExtendedData) 接口更新了自己的信息后，其他在线客户端可通过注册 [onUserInfoUpdated\|_blank](@onUserInfoUpdated) 事件监听用户信息被修改。
...
当用户在一端通过 [deleteConversation\|_blank](@deleteConversation) 接口删除服务端会话后（即 [isAlsoDeleteServerConversation\|_blank](@isAlsoDeleteServerConversation) 为 true），其他在线客户端可通过注册 [onConversationChanged\|_blank](@onConversationChanged) 事件监听会话被删除。
...
当用户在一端通过 [deleteAllConversations\|_blank](@deleteAllConversations) 接口删除全部服务端会话后（即 [isAlsoDeleteServerConversation\|_blank](@isAlsoDeleteServerConversation) 为 true），其他客户端可通过 [onConversationsAllDeleted\|_blank](@onConversationsAllDeleted) 事件监听全部会话被删除。
...
当用户在一端通过 [clearConversationTotalUnreadMessageCount\|_blank](@clearConversationTotalUnreadMessageCount) 清除全部会话未读时，其他客户端可通过 [onConversationTotalUnreadMessageCountUpdated\|_blank](@onConversationTotalUnreadMessageCountUpdated) 事件监听所有会话的未读都被清零的通知。
...
当用户登录新设备后，SDK 不会自动将旧设备中的已有消息同步到新设备上，用户需要主动调用 [queryHistoryMessage\|_blank](@queryHistoryMessage) 接口，才能获取存储于 ZIM 服务端的消息，详情请参考 [获取历史消息\|_blank](!IM-query_Message_Records)。对于存储于旧设备的本地消息，则无法获取。

#### 删除服务端消息

当用户在一端通过 [deleteMessages\|_blank](@deleteMessages)、[deleteAllMessage\|_blank](@deleteAllMessage)、[deleteAllConversationMessages\|_blank](@deleteAllConversationMessages) 接口删除会话的服务端消息后（即 [isAlsoDeleteServerMessage\|_blank](@isAlsoDeleteServerMessage) 为 true），其他在线客户端可通过注册 [onMessageDeleted\|_blank](@onMessageDeleted) 事件监听消息被删除。
...
当用户在一端通过 [sendMessageReceiptsRead\|_blank](@sendMessageReceiptsRead)、[sendConversationMessageReceiptRead\|_blank](@sendConversationMessageReceiptRead) 接口设置消息回执已读后，其他在线客户端可通过注册 [onMessageReceiptChanged\|_blank](@onMessageReceiptChanged)、[onConversationMessageReceiptChanged\|_blank](@onConversationMessageReceiptChanged) 事件监听本帐号已设置消息回执为已读。
...
房间模块相关接口和事件不支持多端登录。用户在 A 设备加入房间后，然后在 B 设备再加入相同的房间后，会把 A 设备踢出房间，A 设备会通过 [onRoomStateChanged\|_blank](@onRoomStateChanged) 回调收到 `event` 为 `KickedOutByMultiDevice`。
...
当用户同时登录设备 A 和 B，用户收到呼叫邀请，在设备 A 上接受邀请（调用 [callAccept\|_blank](@callAccept)）或者拒绝邀请（调用 [callReject\|_blank](@callReject)) 后：
1. 设备 A 可以通过相关操作的回调（[ZIMCallAcceptanceSentCallback\|_blank](@-ZIMCallAcceptanceSentCallback) 或 [ZIMCallRejectionSentCallback\|_blank](@-ZIMCallRejectionSentCallback)） 得知操作结果，关闭邀请弹窗，实现其他业务操作；
2. 设备 B 应当通过 [onCallUserStateChanged\|_blank](@onCallUserStateChanged) 回调，得知本用户的呼叫用户状态（[ZIMCallUserState\|_blank](@-ZIMCallUserState)）是 `Accepted` 还是 `Rejected`，关闭邀请弹窗，实现其他业务操作

各个设备能收到该呼叫内的用户状态变更事件 [onCallUserStateChanged\|_blank](@onCallUserStateChanged) 如下表所示：
