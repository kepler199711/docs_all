...
联系 ZEGO 技术支持，开通多端登录服务，配置多端登录之后，即可在多端调用 {getPlatformData(props,loginMap)} 登录 ZIM SDK，实现流程与单端登录无异。如何登录，详情请参考 [实现基本消息收发](../../Send%20and%20receive%20messages.mdx) 的“登录 ZIM“。 
