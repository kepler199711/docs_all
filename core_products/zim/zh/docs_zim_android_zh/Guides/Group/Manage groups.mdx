...
- {getPlatformData(props,inviteUsersIntoGroupMap)} ：直接邀请用户入群。
...
:::if{props.platform= "iOS"}
<Warning title="Warning">
群内成员调用 {getPlatformData(props,inviteUsersIntoGroupMap)} 接口，邀请其他用户加入群组时：
- 被添加的用户将直接进入群组，无需同意。
- 被添加的用户 userID，必须是真实存在的（即已经通过 {getPlatformData(props,loginWithUserInfoMap)} 接口登录注册过），否则会操作失败。
</Warning>
:::
