...
export const ZIMCombineMessageDetailQueriedCallbackMap = {//错误链接
  ...
  'Web': <a href='@-ZIMCombineMessageDetailQueriedResult' target='_blank'>ZIMCombineMessageDetailQueriedResult</a>,
...
接收合并消息的回调接口与接收普通消息的回调接口一致，请参考 [收发普通消息 - 接收消息](!IM-sendMessages#4_2) 了解具体接口。

<Warning title="注意">

接收消息回调仅返回合并消息的标题、概要、合并 ID 的信息，如需了解合并消息内容，请参考 <a href="#queryCombineMessageDetail">查看合并消息详情。</a> 
</Warning>
...
逐条转发消息本质上就是将已发送成功的消息作为参数发送到其它会话，与发送普通消息所用接口相同，开发者可参考 [收发普通消息 - 发送消息](./Send%20and%20receive%20messages.mdx#发送消息) 了解此接口参数详情。
