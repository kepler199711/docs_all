...
登录 ZIM 后，用户可以监听 {getPlatformData(props,onReceivePeerMessageMap)} 和 {getPlatformData(props,onReceiveGroupMessageMap)} 回调接收单聊会话和群聊会话的新消息，或调用 {getPlatformData(props,queryHistoryMessageMap)} 接口拉取历史消息。
...
- 文本消息：[ZIMTextMessage(1)](@-ZIMTextMessage)
- 图片消息：[ZIMImageMessage(11)](@-ZIMImageMessage)
- 文件消息：[ZIMFileMessage(12)](@-ZIMFileMessage)
- 音频消息：[ZIMAudioMessage(13)](@-ZIMAudioMessage)
- 视频消息：[ZIMVideoMessage(14)](@-ZIMVideoMessage)
- 合并消息：[ZIMCombineMessage(100)](@-ZIMCombineMessage)
- 自定义消息：[ZIMCustomMessage(200)](@-ZIMCustomMessage)
</Note>
...
// 回复一条消息，引用的源消息对象 toOriginalMessage 可以通过 queryHistoryMessage 或者 receivePeerMessage 和 receiveGroupMessage 获取。
...
监听 {getPlatformData(props,onReceivePeerMessageMap)} 和 {getPlatformData(props,onReceiveGroupMessageMap)} 回调在单聊会话和群聊会话中收到新消息后，需要通过判断消息是否存在 {getPlatformData(props,repliedInfoMap)} （源消息基本信息）：
