...
您只需调用 {getPlatformData(props,setConversationMarkMap)} 接口，即可对至多 100 个会话（仅支持**单聊**或**群聊**）设置或取消标记。每个会话至多可以拥有 20 个标记。
...
开发者可以通过调用服务端 API 批量对多名用户设置或取消会话标记，详情请参考服务端 API 文档 [设置会话标记](/zh/zim-server/conversation/set-conversation-marks)。
