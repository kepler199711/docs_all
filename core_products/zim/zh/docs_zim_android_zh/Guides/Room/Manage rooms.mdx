...
export const CONNECTINGMap = {
  'Android': <a href='@connecting-ZIMRoomState' target='_blank'>CONNECTING</a>,
  'Flutter': <a href='https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIMRoomState.html' target='_blank'>connecting</a>,
  'Web,RN,uniapp': <a href='@connecting-ZIMRoomState' target='_blank'>connecting</a>,
  'iOS,mac': <a href='@ZIMRoomStateConnecting-ZIMRoomState' target='_blank'>ZIMRoomStateConnecting</a>,
  "window": <a href='@zim_room_state_connecting-ZIMRoomState' target='_blank'>ZIM_ROOM_STATE_CONNECTING</a>
}
...
- 完整流程请参考下图：
    ...
    | 触发时机                                           | roomID 值 | state 值 | event 值 | 事件意义       |
    | ------------------------------------------------- | --------- | -------- | -------- | -------------- |
    | 调用 {getPlatformData(props,switchRoomMap)} 接口后 | Room2     | {getPlatformData(props,CONNECTINGMap)}        | 11       | 正在连接 Room2 |
