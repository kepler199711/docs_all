...
- 开发者可以通过注册 [setEventHandler](@setEventHandler) 监听，用于接收相关通知（接收房间消息、连接状态、token 即将过期等通知）。
...
1. 客户端 A、B 分别创建自己的 ZIM 实例，并注册 [setEventHandler](@setEventHandler) 监听的 [onReceivePeerMessage](@onReceivePeerMessage) 回调接口，用于接收单聊消息通知。
2. 客户端 A、B 分别登录 ZIM SDK。
3. 客户端 A 调用 [sendMessage](@sendMessage) 接口，并设置 `conversationType` 为 `zim::ZIMConversationType::ZIM_CONVERSATION_TYPE_PEER` 发送一条单聊消息到客户端 B。
...
开发者可以通过 [ZIMMediaMessageSendNotification](@-ZIMMediaMessageSendNotification) 的`onMediaUploadingProgress`回调，接收富媒体消息的下载进度的相关通知。
...
在用户被提醒时，会收到 [onConversationChanged](@onConversationChanged) 回调，即可获取当前 [ZIMConversation](@-ZIMConversation) 的最新 [mentionedInfoList](@mentionedInfoList-ZIMConversation)。
...
会话内所有用户都可以通过 [ZIMMessage](@-ZIMMessage) 的 [mentionedUserIDs](@mentionedUserIDs-ZIMMessage) 参数获取具体提醒用户列表，可此接口仅能返回 @ 消息发送用户通过 [ZIMMessage](@-ZIMMessage) 的 [mentionedUserIDs](@mentionedUserIDs-ZIMMessage) 参数传入的用户列表。
...
会话内所有用户都可以通过 [ZIMMessage](@-ZIMMessage) 的 [isMentionAll](@isMentionAll-ZIMMessage) 参数，确认消息是否为全员提醒消息。
...
通过 [onBroadcastMessageReceived](@onBroadcastMessageReceived) 回调，即可接收全员推送消息。
