...
<Note title="Note">本功能仅限在 iOS 设备上实现。</Note>
...
// 设置 pushConfig 的属性
pushConfig.title = "系统通话标题"; // 设置系统通话的标题
pushConfig.payload = "业务需要的透传字段"; // 设置业务透传的字段
pushConfig.resourcesID = "联系 ZEGO 技术支持配置的 resourcesID"; // 设置 resource ID，需联系 ZEGO 技术支持配置

let voIPConfig = new ZIMVoIPConfig(); 

// 配置 VoIP 设置
voIPConfig.iOSVoIPHandleType = ZIMCXHandleType.generic; // 设置 iOS VoIP 的 Handle 类型为通用类型
voIPConfig.iOSVoIPHandleValue = "Li hua"; // 设置发送方联系人信息
voIPConfig.iOSVoIPHasVideo = true; // 设置是否为视频通话
