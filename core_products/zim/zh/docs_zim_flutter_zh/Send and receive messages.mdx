...
1. 打开 “pubspec.yaml” 文件，以 “pub” 形式，添加 “zego_zim” 依赖：
...
3. 如果您使用的是 Web 平台，还需要在您的 Web 项目的 index.html 文件中，添加如下代码，引入 Web SDK。
...
例如，客户端 A、B 分别调用 [create](https://pub.dev/documentation/zego_zim/latest/zego_zim/ZIM/create.html) 接口，传入在 [1 前提条件](!Quick_Starts/Implementation_of_sending_and_receiving#1) 中获取到的 AppID、AppSign（开发 Web 平台应用时无需使用 “appsign”，为防止暴露，请勿传入），创建了 A、B 的实例：
...
- 如果您使用的是 “Token 鉴权”，请参考 [使用 Token 鉴权](!common_features/Authenticate_users_with_tokens) 文档，获取 Token 后，并在登录 ZIM 时传入可选参数 Token，鉴权通过后才能登录成功。
</Warning>
...
<Frame width="512" height="auto" caption=""><img src="https://storage.zego.im/sdk-doc/Pics/ZIM/flutter/peerMessage_Flutter_new.png" /></Frame>
