...
开发者使用 ZIM 提供的“离线推送”功能之前，请参考本文，接入鸿蒙 next 的原生离线推送功能。  

## 前提条件

在接入 `鸿蒙 next` 的离线推送通道之前，请确保：
...
- 已完成如下步骤：
    ...
    2. 参考 https://developer.huawei.com/consumer/cn/doc/harmonyos-guides/push-config-setting ,开通推送服务。
    3. 参考 https://developer.huawei.com/consumer/cn/doc/start/api-0000001062522591#section14931730144919
       申请服务账号密钥，保存生成的 JSON 文件。

## 使用步骤

请在 [ZEGO 控制台](https://console.zego.im/) 自助配置 ZIM 离线推送（详情请参考 [项目管理 - 即时通讯 - 离线推送配置](https://doc-zh.zego.im/article/16233)），将申请服务账号获得的 JSON 文件提交到此处。若无法配置，请联系 ZEGO 技术支持处理。
...
2. 通过 ZIM 发起推送时，将会按照 IM 类型的自分类类型来进行推送。