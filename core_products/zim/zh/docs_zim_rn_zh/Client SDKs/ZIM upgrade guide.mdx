...
<CodeGroup>
```javascript 2.19.0版本用法（下载组合消息指定媒体内容） {14-17,19,29}
...
```javascript 2.19.0用法（下载多媒体消息） {6,15}
...
<CodeGroup>
```javascript 2.19.0版本用法 {10,14,19}
const imageMessage = new ZIMMessageBase({
    type: ZIMMessageType.Image,
    fileLocalPath: file
})
