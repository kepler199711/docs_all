...
| 自定义点击跳转 | 支持自定义用户点击消息推送通知后的跳转目标页面。用户点击行为会触发 [notificationClicked](@notificationClicked) 回调，开发者即可获取透传信息并执行页面跳转逻辑，无需关注推送通道的差异。实现流程请参考 [自定义点击跳转](./../Offline%20push%20notifications/Custom%20click%20jump%20implementation.mdx)。<Note title="说明">本功能仅适用于除 FCM 以外的 Android 推送通道和 Apple APNs 推送。</Note> | [notificationClicked](@notificationClicked) |
