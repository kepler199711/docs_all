...
根据会话类型（单聊、房间、群组），消息接收客户端监听 [receivePeerMessage](@receivePeerMessage)、[receiveGroupMessage](@receiveGroupMessage)、[receiveRoomMessage](@receiveRoomMessage)，接收语音消息的相关通知，然后可以调用 [downloadMediaFile](@downloadMediaFile) 接口，下载音频文件到本地。
