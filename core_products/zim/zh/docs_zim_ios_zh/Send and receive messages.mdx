...
<Tabs>
<Tab title="2.3.0 及以后版本">
2.3.0 文案及代码
</Tab>
<Tab title=" 2.3.0 以前版本">
...
详细的接口介绍，请参考 [tokenWillExpire](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-token-will-expire)、[connectionStateChanged](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-connection-state-changed-event-extended-data)、[receivePeerMessage](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-peer-message-from-room-id)。

#### 4. 登录 ZIM

创建实例后，客户端 A 和 B 需要登录 ZIM，只有登录成功后才可以开始发送、接收消息、更新 Token 等。

首先，请创建用户个人登录配置（[ZIMLoginConfig](@-ZIMLoginConfig) 对象）；创建完毕后，调用 [loginWithUserID](/article/api?doc=zim_API~objective-c_ios~protocol~ZIM#login-with-user-id-config-callback-2) 接口，传入上述对象，即可登录 ZIM。
...
- 如果您使用的是 “Token 鉴权”，请参考 [使用 Token 鉴权](!common_features/Authenticate_users_with_tokens) 文档，获取 Token 后，并在登录 ZIM 时传入 [ZIMLoginConfig.Token](@token-ZIMLoginConfig)，鉴权通过后才能登录成功。
</Warning>

```objc
// 请将以下代码替换为 loginWithUserID 的调用示例
...
客户端 B 登录 ZIM 后，将会收到在 [setEventHandler](@setEventHandler) 回调中设置的 [receivePeerMessage](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-peer-message-from-room-id) 监听接口，收到客户端 A 发送过来的消息。
...
- 接收消息时：
    - 单聊消息（Peer 类型），通过 [receivePeerMessage](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-peer-message-from-room-id) 回调接收。
    - 房间消息（Room 类型），通过 [receiveRoomMessage](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-room-message-from-room-id) 回调接收。
    - 群组消息（Group 类型），通过 [receiveGroupMessage](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-receive-group-message-from-group-id) 回调接收。
</Note>
