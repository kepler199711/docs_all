...
客户端 B 登录 ZIM 后，将会收到在 [setEventHandler](@setEventHandler) 回调中设置的 [peerMessageReceived](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-peer-message-received-info-from-user-id) 监听接口，收到客户端 A 发送过来的消息。
...
<MarkSendMessageEvent platform="ios" />
