...
根据会话类型（单聊、房间、群组），消息接收客户端监听 [peerMessageReceived](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-peer-message-received-from-user-id)、[groupMessageReceived](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-group-message-received-from-group-id)、[roomMessageReceived](/article/api?doc=zim_API~objective-c_ios~protocol~ZIMEventHandler#zim-room-message-received-from-room-id)，接收语音消息的相关通知，然后可以调用 [downloadMediaFileWithMessage](@downloadMediaFileWithMessage) 接口，下载音频文件到本地。
