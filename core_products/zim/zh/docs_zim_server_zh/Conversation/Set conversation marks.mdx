...
<td>SetMarkGroup</td>
<td>Object of <a href="#ConvMarkInfo">ConvMarkInfo</a></td>
<td>群聊会话列表。key 为会话 ID，value 为 ConvMarkInfo 结构。列表最多可包含 100 个会话。</td>
</tr>
<tr data-row-level="6">
<td>Action</td>
<td>Bool</td>
...
<td>Number</td>
<td>返回码。
<Note title="Note">当您发起请求设置或取消多个会话的标记时：<ul><li>只需成功对至少一个会话完成操作，Code 都会返回 0，表示成功。此时请参考 FailList 中的具体信息，了解失败操作详情。</li><li>如果所有会话的操作都失败，Code 会返回相关返回码，具体请参考 <a href="https://doc-zh.zego.im/article/12229">全局返回码</a>。</li></ul></Note></td>
</tr>
<tr data-row-level="3">
<td>Message</td>
<td>String</td>
