...
<tr>
<td><a href="/article/api?doc=zim_API~javascript_wxxcx~interface~ZIMEventHandler#conversation-changed" target="_blank" rel="noopener">conversationChanged</a></td>
...
<td><a href="/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#conversation-changed" target="_blank" rel="noopener">conversationChanged</a></td>
...
<td>是</td>
<td>会话 ID。
<ul><li>单聊时，conversationID 即是对端用户的 userID。</li><li>群聊时，conversationID 即是群组的 groupID。</li></ul></td>
</tr>
...
<td>Number</td>
<td>返回码。
<Note title="Note">当您发起请求修改多个会话的置顶状态时：<ul><li>只需成功修改 1 个或以上会话的状态，Code 都会返回 0，表示成功。此时请参考 ErrorList 中的具体信息，了解会话修改状态失败详情。</li><li>如果所有会话的状态都修改失败，Code 会返回相关返回码，具体请参考 <a href="https://doc-zh.zego.im/article/12229">全局返回码</a>。</li></ul></Note></td>
</tr>
<tr data-row-level="3">
<td>Message</td>
<td>String</td>
