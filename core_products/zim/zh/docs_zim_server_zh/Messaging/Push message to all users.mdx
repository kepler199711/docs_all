...
<Note title="说明">通过本接口发送的消息不会导致会话产生，也不会被保存。</Note>
...
<tr>
<td><a href="/article/api?doc=zim_API~javascript_wxxcx~interface~ZIMEventHandler#broadcast-message-received" target="_blank" rel="noopener">broadcastMessageReceived</a></td>
...
<td><a href="/article/api?doc=zim_API~javascript_uni-app~interface~ZIMEventHandler#broadcast-message-received" target="_blank" rel="noopener">broadcastMessageReceived</a></td>
