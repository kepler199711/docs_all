...
<td>Number</td>
<td>返回码。
<Note title="Note">当您发起请求同时将多名指定用户添加为好友时：<ul><li>如果成功添加 1 名或更多好友，Code 都会返回 0。此时请参考 ErrorList 中的具体信息，确认操作结果。</li><li>如果添加全部指定用户为好友失败，Code 会返回相关返回码，具体请参考 <a href="!Server_APIs/Return_Codes" target="_blank">全局返回码</a>。</li></ul></Note></td>
</tr>
<tr data-row-level="3">
<td>Message</td>
<td>String</td>
