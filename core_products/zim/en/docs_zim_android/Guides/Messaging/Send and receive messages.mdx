...
- {getPlatformData(props,mentionedUserIDsMap)}: Notifies specific users (including users outside of the conversation) to view the message. The length of the userID list passed in should be up to 50. If you need to increase this limit, please contact the ZEGOCLOUD technical support team.
- {getPlatformData(props,isMentionAllMap)}: Notifies all other users within the conversation to view the message.
...
All users in the conversation can call the `getMentionedUserIDs` interface. Get the specific reminder user list, but this interface can only return the user list passed in by calling the `setMentionedUserIDs` interface.
