...
<ul><li>If a group message is sent from the **client**, obtain `MsgSeq` by using the [Callback on message sent|_blank](./../Callbacks/Message%20sent.mdx) operation.</li><li>If a group message is sent by using the server API [`SendRoomMessage`|_blank](./Send%20room%20messages.mdx), obtain `MsgSeq` in the response data.</li></ul></td>
</tr>
<tr>
<td>Payload</td>
<td>String</td>
